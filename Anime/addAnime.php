...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Handle file upload
    $target_dir = "../uploads/anime/"; // Set the target directory for cover image
    if (!is_dir($target_dir)) {
        mkdir($target_dir, 0777, true); // Create the directory if it doesn't exist
    }

    $cover_image = $target_dir . basename($_FILES["cover_image"]["name"]);
    move_uploaded_file($_FILES["cover_image"]["tmp_name"], $cover_image); // Move the uploaded image to the target directory

    // Save data into the database
    $sql = "INSERT INTO anime (title, description, release_year, genre, rating, cover_image) 
            VALUES ('$title', '$description', '$release_year', '$genre', '$rating', '$cover_image')";
